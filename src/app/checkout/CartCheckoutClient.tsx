...
export default function CartCheckoutClient() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        formData.append('cartItems', JSON.stringify(items.map(i => ({
            id: i.id,
            quantity: i.quantity,
            price: i.price
        }))));
    ...
    return (
        <div className="container animate-fade-in" style={{ paddingBottom: '4rem' }}>
            <div style={{ textAlign: 'center', marginBottom: '3rem', marginTop: '1rem' }}>
                ...
                <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem' }}>Complete your order with {items.length} item(s)</p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(400px, 1fr) 350px', gap: '2rem', alignItems: 'start' }}>
                ...
                <div className="glass-panel" style={{ padding: '2.5rem', position: 'sticky', top: '90px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1.5rem' }}>
                        {items.map(item => (
                            <div key={item.id} style={{ display: 'flex', gap: '1rem' }}>
                                <div style={{ width: '50px', height: '50px', borderRadius: 'var(--radius-sm)', backgroundColor: '#111', overflow: 'hidden', flexShrink: 0 }}>
                                    {item.imageUrl ? (
                                ...
                                <div style={{ flex: 1 }}>
                                    <p style={{ fontWeight: 600, margin: 0, fontSize: '0.85rem', lineHeight: 1.2 }}>{item.name}</p>
                                    <p style={{ color: 'var(--text-secondary)', margin: 0, fontSize: '0.8rem', marginTop: '0.2rem' }}>{item.quantity}x unit</p>
                                </div>
                                <span style={{ fontWeight: 500, fontSize: '0.9rem', flexShrink: 0 }}>
                    ...
                    <div style={{ borderTop: '1px solid var(--border-light)', paddingTop: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <span className="text-gradient" style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>Rp {grandTotal.toLocaleString('id-ID')}</span>
                    </div>
                </div>
